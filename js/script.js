	...
	if (linksMass.length > 99) {
		...
		linksMass.forEach((link, index) => {
			...
			if (index < 99) {
				...
				// Удаляем атрибут style из html, чтобы срабатывали свойства CSS
				link.querySelector('.body-li').removeAttribute("style");

			} else {
				if (document.documentElement.clientWidth < 565) {
					link.querySelector('.body-li').style.fontSize = '11px';
					// иначе, если номер 3х значный , размер 30рх
				} else if (document.documentElement.clientWidth >= 565 && document.documentElement.clientWidth < 675) {
					link.querySelector('.body-li').style.fontSize = '17px';
				} else if (document.documentElement.clientWidth >= 675 && document.documentElement.clientWidth < 1309) {
					link.querySelector('.body-li').style.fontSize = '26px';
				} else if (document.documentElement.clientWidth >= 1309) {
		...
		// просто удаляем атрибут style в html
